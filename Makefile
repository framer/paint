...
API_TARGET=dist/framer-paint.api.json
API_REVIEW_FILE=api/framer-paint.api.ts
...
test: bootstrap $(API_TARGET)
	yarn test
	yarn tsc dist/framer-paint.d.ts --skipLibCheck

test-ci: bootstrap $(API_TARGET)
	mkdir -p $(TEST_REPORT_PATH)
	JEST_JUNIT_OUTPUT=$(TEST_REPORT_PATH)/framer-paint.xml yarn test-ci --ci --reporters=jest-junit
	yarn tsc dist/framer-paint.d.ts --skipLibCheck
